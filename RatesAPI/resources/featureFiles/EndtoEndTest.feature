...
Feature: Validate API’s are fit for purpose in the use of the exchange rate for financial reasons
  ...
  Background: 
    Given Exchange Rates API is up and running

		Scenario: To Verify the success status for Specific date Foreign Exchange Rates with Symbols and Baseapi URL
    	When Hit the API with end point as "/2010-01-12?base=USD&symbols=GBP"
    	Then API should return the status code as 200
    
    Scenario: assert the current date in response while passing future date 
    	When Hit the API with end point as "/2040-01-12"
